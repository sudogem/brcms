...
$where = " WHERE autoNumber = '$categoryID'";
...
$rs2 = get_table_data('news_segments');
while($data2 = mysql_fetch_assoc($rs2)) {
	$segments .= '<li><a href="segments.php?segmentID='.$data2['autoNumber'].'">'.$data2['segmentTitle'].'</li>';
}
...
$fullstory = '<p><a href="view_segment_detail.php?segment=' .$_REQUEST['segmentID'] . '" >FULLSTORY &gt;&gt;</a></p>';
...
$tags = array(
		...
		'{SEGMENT_TITLE}'		=> $data['segmentTitle'],	#$heading_category,
		'{SEGMENT_TOPIC}'		=> 'Topic: '.$data['segmentTopic'],     #$headline[0]->title ,
		'{SEGMENT_ANCHOR}'		=> 'Anchor Man: '.$data['segmentAnchor'], 	#'by '. getArticle_authors_info( $headline[0]->articleID, 'fullname' )  . ' / BR</p>' ,
		'{SEGMENT_BODY}' 		=>  makeAShortIntro( $data['segmentContent'] ), #makeAShortIntro( $headline[0]->article_body ),
		'{SEGMENT_SCHEDULE}'	=> $data['segmentSchedule'],
