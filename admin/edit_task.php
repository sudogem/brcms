...
$assignedto  = $_SESSION['assignedto'];
 ...
 */
$prioritylist = array( "Low", "Normal", "High" );
foreach( $prioritylist as $field => $data ) {
	if ( $data == $priority ) {
 ...
 */
$priority = array( "Not Started", "In Progress", "Completed" );
	...
	if ( $data == $status ) {
		switch( $data ) {
			case 'Not Started':
				...
				break;
			case 'In Progress':
				$optstatus .= '<span class="viola">';
				break;
			case 'Completed':
				$optstatus .= '<span class="green">';
				break;
		}
		$optstatus .= $data;
		$optstatus .= '</span>';
...
$tpl = new template_parser( '../templates/edit_task.tpl.php' );
$tags = array(
		'{SUBJECT}'			=> $subject ,
