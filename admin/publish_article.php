...
if ( $clickeditem ) {
		...
		$sql = " update articles ";
		$sql .= " set articles.stageID='6' , status = 'live' , ";
		...
		$_SESSION['task'] = 'publish';
		$_SESSION['title'] = $title;
		$subject = " PUBLISHED : " . $_SESSION['title'];
		$message = " The article was successfully published. Thanks =) ";
	...
	foreach( $checkitem as $idx => $value ) {
		...
		// Also we have to update on the article versions...
		$sql = " update article_versions ";
		$sql .= " set article_versions.stageID = '6' , status = 'live' , ";
		$sql .= " status = 'published' ,";	
		$sql .= " dateline= '$date_published' ,";
		$sql .= " published_day = '$d' ,";
		$sql .= " published_month = '$m' ,";
